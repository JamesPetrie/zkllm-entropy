...
if __name__ == '__main__':
    ...
    if args.goldilocks:
        ...
        compilation_error = os.system('make gold_commit-param')
    else:
        commit_bin = './commit-param'
        compilation_error = os.system('make commit-param')
    if compilation_error:
        print(f"Error compiling {commit_bin}")
        exit(1)
    ...
    for i, layer in enumerate(model.model.layers):
        for j, w in layer.named_parameters():
            ...
            commitment_path = f"./zkllm-workdir/Llama-2-{args.model_size}b/layer-{i}-{j}{com_suffix}"
            save_weight_int(w_out, int_bin_path)
            if args.goldilocks:
                # FRI: no pp file, 4 args
                if len(w_out.shape) == 2:
                    os.system(f'{commit_bin} {int_bin_path} {commitment_path} {w_out.shape[0]} {w_out.shape[1]}')
                else:
                    os.system(f'{commit_bin} {int_bin_path} {commitment_path} {w_out.shape[0]} 1')
            else:
                # Pedersen: pp file, 5 args
                if len(w_out.shape) == 2:
                    os.system(f'{commit_bin} {pp_path} {int_bin_path} {commitment_path} {w_out.shape[0]} {w_out.shape[1]}')
                else:
                    os.system(f'{commit_bin} {pp_path} {int_bin_path} {commitment_path} {w_out.shape[0]} 1')
