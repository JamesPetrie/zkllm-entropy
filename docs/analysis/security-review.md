...
### 3.5 ZK Masking Implementation (2026-04-03 Update)

Branch `zk-masking-implementation` adds per-sumcheck zero-knowledge via two mechanisms:

1. **Vanishing polynomial masking**: Z_f(X) = f(X) + Σ c_i·X_i(1-X_i). The vanishing terms are zero on {0,1}^k, so committed data is unchanged but evaluations at random non-boolean points (revealed during sumcheck) are hidden.

2. **XZZ+19 transcript masking**: Each round polynomial becomes s(X) = g(X) + ρ·p(X), where p is a random separable polynomial. This hides the round messages themselves.

**What this addresses**: Sumcheck round messages no longer reveal weighted sums of private inputs. The degree-4 masked round polynomials (5 evaluations per round) are statistically indistinguishable from random degree-4 polynomials satisfying the sum constraint.

**What this does NOT address**: FRI blinding (§3.1), per-position value leakage (§3.2), and cross-layer junction leakage. These require Techniques 2 and 3 from `plan-zk-blinding.md`.

**Known gaps**: ρ is currently generated by the prover, not derived from a Fiat-Shamir transcript or verifier challenge. No stacked ZK verifier exists yet. See `plan-zk-blinding.md` for full status.
...
|-------|-----------|--------|
| No FRI blinding | fri.cu | Open |
| Per-position values in proof file | zkentropy.cu / verify_entropy.py | Open |
| Proof reveals token-level model confidence | Architecture | Open |
| No sumcheck transcript masking | proof.cu | **Partially addressed** (see §3.5) |
